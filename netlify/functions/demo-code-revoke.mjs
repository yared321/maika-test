/**
 * Admin API: revoke (delete) a dynamic demo access code from Redis.
 *
 * Behavior:
 * - Requires POST with x-admin-key (DEMO_CODE_ADMIN_SECRET).
 * - Accepts { code } JSON body.
 * - Deletes usage/meta keys for the provided code.
 * - Returns whether keys were actually removed.
 *
 * Notes:
...
export const handler = async (event) => {
  ...
  const revoked = await revokeDynamicAccessCode(code);

  if (!revoked.ok) {
    if (revoked.reason === "invalid_code_format") {
      return jsonResponse(400, { ok: false, error: "Invalid access code format." }, corsAllow);
    }
    return jsonResponse(503, { ok: false, error: "Could not revoke code." }, corsAllow);
  }
